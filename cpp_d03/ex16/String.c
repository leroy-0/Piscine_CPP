...
static void join_c(String *this, char delim, char const **tab)
{
	...
	while (tab && tab[x])
	{
		malloc_size += strlen(tab[x]);
		x++;
	}

	this->str = malloc(sizeof(char) * (malloc_size + x + 1 ));
	this->str[0] = '\0';
	mydelim = malloc(sizeof(char) * 2);
  	mydelim[0] = delim;
  	mydelim[1] = '\0';
	...
	while (tab && tab[x])
	{
		strcat(this->str, tab[x]);
		if (tab[x + 1])
			strcat(this->str, mydelim);
		x++;
	}
	free(mydelim);
}
...
String 	*substr(String *this, int offset, int lenght)
{
	...
	if (offset >= 0 && offset < (int)(strlen(this->str) - 1))
		off =  offset;
	else if (offset >= 0 && offset > (int)(strlen(this->str) - 1))
		off = strlen(this->str) - 1;
	else if (offset < 0 && (offset * - 1) < (int)(strlen(this->str) - 1))
		off = strlen(this->str) - 1 + offset;
	else
		off = 0;

	printf("%d\n", off);

	substr = malloc(sizeof(char) * (strlen(this->str) + 1));

	if (lenght >= 0)
	{
		while (this->str[off + x] && x < lenght)
		{
			substr[x] = this->str[off + x];
			x++;
		}
		substr[x] = '\0';
	}
	...
	{
		while ((off - x ) >= 0 && this->str[off - x] && x < (lenght * -1))
		{
			substr[x] = this->str[off - x];
			x++;
		}
		...
		StringInit(newString, substr);

		x = 0;
		off = strlen(substr) - 1;
		while (substr[x])
		{
			newString->str[x] = substr[off];
			off--;
			x++;
		}
		newString->str[x] = '\0';
